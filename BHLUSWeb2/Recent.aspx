    ...
    <div id="content" class="column-wrap clearfix">
        ...
        <section class="recentfeed">
			<asp:Repeater ID="rptRecent" runat="server">
				<ItemTemplate>
					<li>
                        <a target="<%# string.IsNullOrWhiteSpace(Eval("ExternalUrl").ToString()) ? "_self" : "_blank" %>" <%# string.IsNullOrWhiteSpace(Eval("ExternalUrl").ToString()) ? "" : "rel=\"noopener noreferrer\"" %> href="/item/<%# Eval("ItemID") %>" class="booktitle<%# string.IsNullOrWhiteSpace(Eval("ExternalUrl").ToString()) ? "" : " ExtLink" %>" title="Book"><%# Eval("FullTitleExtended") %> <%# Eval("Volume") %></a>
