...
BookReader.prototype.init = function () {
    ...
    if ($('#BRcontainer')[0].clientWidth === 0 || $('#BRcontainer')[0].clientHeight === 0) {
    ...
    $(window).bind('resize', this, function (e) {
        if (1 === e.data.mode) {
            ...
            if (e.data.twoPage.autofit) {
                ...
                if (e.data.twoPage.totalWidth < $('#BRcontainer')[0].clientWidth) {
                    center.percentageX = 0.5;
                    doRecenter = true;
                }
                if (e.data.twoPage.totalHeight < $('#BRcontainer')[0].clientHeight) {
...
BookReader.prototype.drawLeafsOnePage = function () {
    ...
    var scrollTop = $('#BRcontainer')[0].scrollTop;
    ...
    var viewWidth = $('#BRcontainer')[0].scrollWidth;
...
BookReader.prototype.drawLeafsThumbnail = function (seekIndex) {
    ...
    var viewWidth = $('#BRcontainer')[0].scrollWidth - 20; // width minus buffer
    ...
    var pageViewBuffer = Math.floor(($('#BRcontainer')[0].scrollWidth - maxRight) / 2) - 14;
    ...
    if ($.browser.msie && (parseInt($.browser.version, 10) === 8 || parseInt($.browser.version, 10) === 7)) {
        for (var i = 0; i < $('.BRpagedivthumb img').length; i++) {
            $('.BRpagedivthumb img').width(leafWidth);
            $('.BRpagedivthumb img').height(leafHeight);
        }
    }
};
...
BookReader.prototype.drawLeafsTwoPage = function () {
    ...
    //var scrollTop = $('#BRtwopageview')[0].scrollTop;
...
BookReader.prototype.resizePageView1up = function () {
    var i;
    var viewHeight = 0;
    var viewWidth = $('#BRcontainer')[0].clientWidth;

    //var oldScrollTop = $('#BRcontainer')[0].scrollTop;
    //var oldScrollLeft = $('#BRcontainer')[0].scrollLeft;
    ...
    this.onePageCalculateReductionFactors($('#BRcontainer')[0].clientWidth,
        $('#BRcontainer')[0].clientHeight);
    ...
    $('#BRcontainer')[0].scrollTop = newTop;

    // We use clientWidth here to avoid miscalculating due to scroll bar
    var newCenterX = oldCenterX * (viewWidth / oldPageViewWidth);
    var newLeft = newCenterX - $('#BRcontainer')[0].clientWidth / 2;
    newLeft = Math.max(newLeft, 0);
    $('#BRcontainer')[0].scrollLeft = newLeft;
...
BookReader.prototype.centerX1up = function () {
    var centerX;
    if ($('#BRpageview').width() < $('#BRcontainer')[0].clientWidth) { // fully shown
        centerX = $('#BRpageview').width();
    } else {
        centerX = $('#BRcontainer')[0].scrollLeft + $('#BRcontainer')[0].clientWidth / 2;
    }
...
BookReader.prototype.centerY1up = function () {
    var centerY = $('#BRcontainer')[0].scrollTop + $('#BRcontainer').height() / 2;
...
BookReader.prototype.centerPageView = function () {
    var scrollWidth = $('#BRcontainer')[0].scrollWidth;
    var clientWidth = $('#BRcontainer')[0].clientWidth;
    if (scrollWidth > clientWidth) {
        $('#BRcontainer')[0].scrollLeft = (scrollWidth - clientWidth) / 2;
    }
};
...
BookReader.prototype.jumpToIndex = function (index, pageX, pageY) {
    if (this.constMode2up === this.mode) {
        ...
        if ($('#BRcontainer')[0].scrollTop === leafTop) {
            this.loadLeafs();
        } else {
        ...
        if (pageY) {
            ...
            offset -= $('#BRcontainer')[0].clientHeight >> 1;
            leafTop += offset;
        } else {
        ...
        if (pageX) {
            ...
            offset -= $('#BRcontainer')[0].clientWidth >> 1;
            leafLeft += offset;
        } else {
...
BookReader.prototype.switchMode = function (mode) {
    ...
    if (1 === mode) {
        this.onePageCalculateReductionFactors($('#BRcontainer')[0].clientWidth, $('#BRcontainer')[0].clientHeight);
...
BookReader.prototype.prepareTwoPageView = function (centerPercentageX, centerPercentageY) {
    ...
    if (this.twoPage.totalWidth < $('#BRcontainer')[0].clientWidth) {
        centerPercentageX = 0.5;
    }
    if (this.twoPage.totalHeight < $('#BRcontainer')[0].clientHeight) {
...
BookReader.prototype.getIdealSpreadSize = function (firstIndex, secondIndex) {
    ...
    var maxLeafEdgeWidth = parseInt($('#BRcontainer')[0].clientWidth * 0.1);
...
BookReader.prototype.getSpreadSizeFromReduce = function (firstIndex, secondIndex, reduce) {
    ...
    var totalLeafEdgeWidth = parseInt(this.numLeafs * 0.1);
    var maxLeafEdgeWidth = parseInt($('#BRcontainer')[0].clientWidth * 0.1); // $$$ Assumes leaf edge width constant at all zoom levels
...
BookReader.prototype.onePageGetAutofitWidth = function () {
    //var widthPadding = 20;
    return (this.getMedianPageSize().width + 0.0) / ($('#BRcontainer')[0].clientWidth - this.padding); //widthPadding * 2);
};

BookReader.prototype.onePageGetAutofitHeight = function () {
    return (this.getMedianPageSize().height + 0.0) / ($('#BRcontainer')[0].clientHeight - this.padding); // make sure a little of adjacent pages show
};
...
BookReader.prototype.twoPageSetCursor = function () {
    if (($('#BRtwopageview').width() > $('#BRcontainer')[0].clientWidth) ||
        ($('#BRtwopageview').height() > $('#BRcontainer')[0].clientHeight)) {
...
BookReader.prototype._scrollAmount = function () {
    if (this.constMode1up === this.mode) {
        ...
        return parseInt($('#BRcontainer')[0].clientHeight - this.getPageHeight(this.currentIndex()) / this.reduce * 0.03);
    }

    return parseInt(0.9 * $('#BRcontainer')[0].clientHeight);
};
...
BookReader.prototype.flipLeftToRight = function (newIndexL, newIndexR) {
    ...
    var right = $('#BRtwopageview')[0].clientWidth - left - $(this.prefetchedImgs[leftLeaf]).width() + $('#BRtwopageview').offset().left - 2 + 'px';
...
BookReader.prototype.prepareFlipRightToLeft = function (nextL, nextR) {
    ...
    var leftPage = $(this.prefetchedImgs[nextL]).css({
        ...
        right: $('#BRtwopageview')[0].clientWidth - gutter + 'px',
        top: top + 'px',
        height: this.twoPage.height,
...
BookReader.prototype.twoPageGetViewCenter = function () {
    ...
    center.percentageX = (containerOffset.left - viewOffset.left + ($('#BRcontainer')[0].clientWidth >> 1)) / this.twoPage.totalWidth;
    center.percentageY = (containerOffset.top - viewOffset.top + ($('#BRcontainer')[0].clientHeight >> 1)) / this.twoPage.totalHeight;
...
BookReader.prototype.twoPageCenterView = function (percentageX, percentageY) {
    ...
    var containerClientWidth = $('#BRcontainer')[0].clientWidth;
    var intoViewX = percentageX * viewWidth;

    var viewHeight = $('#BRtwopageview').height();
    var containerClientHeight = $('#BRcontainer')[0].clientHeight;
...
BookReader.prototype.showEmbedCode = function () {
    ...
    $(this.embedPopup).css({
        position: 'absolute',
        top: '20px',
        left: ($('#BRcontainer')[0].clientWidth - 400) / 2 + 'px',
        width: '400px',
        padding: "20px",
...
BookReader.prototype.initToolbar = function (mode, ui) {
    ...
    jToolbar.find('.zoom_out').click(function (e) {
        br.zoom(-1);
        e.preventDefault();
    });

    jToolbar.find('.zoom_in').click(function (e) {
        br.zoom(1);
        e.preventDefault();
    });

    jToolbar.find('.one_page_mode').click(function (e) {
        br.switchMode(1);
    ...
    jToolbar.find('.two_page_mode').click(function (e) {
        br.switchMode(2);
        $(this).siblings().removeClass('active');
        $(this).addClass('active');
        e.preventDefault();
    ...
    jToolbar.find('.thumbnail_mode').click(function (e) {
        br.switchMode(3);
        $(this).siblings().removeClass('active');
        $(this).addClass('active');
        e.preventDefault();
...
BookReader.prototype.updateNavPageNum = function (index) {
    ...
    //scs added
    //update page list
    $('#lstPages')[0].selectedIndex = index;
    //update names
    try {
    ...
    pageNum.css({
        'margin-left': (($('.ui-slider-handle').width() / 2) - pageNum.width()) / 2
    });
};
...
BookReader.prototype.positionPageToolBox = function (pageToolBox) {
    ...
    if (((parentTop - containerScrollTop - container.height() > 0) || (top > maxTop + info.origHeight)) && pageToolBox.hasClass('open')) {
    ...
    if (br.mode !== 3) {
        ...
        var origRight = (br.mode === 1) ? 0 : (parent.parent().offset().left + parent.parent().width()) - (parent.offset().left + parent.width());
        var maxRight = (origRight + parent.width() - info.origWidth);
        var right = (parentLeft + parent.width()) - (containerScrollLeft + containerWidth) + (origRight + $.getScrollbarWidth());
