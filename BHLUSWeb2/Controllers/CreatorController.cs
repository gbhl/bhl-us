...
namespace MOBOT.BHL.Web2.Controllers
{
    public class CreatorController : Controller
    {
        ...
        public ActionResult Index(int? creatorId, string sort, int? bpg, int? ppg, int? psize)
        {
            ...
            {
                return Redirect("~/authornotfound");
            }
            else
            {
                ...
                var bookResults = bhlProvider.TitleSelectByAuthorPaged((int)creatorId, model.BookPage, model.NumPerPage, model.Sort);
                model.TotalBooks = bookResults.Item1;
                model.BookResults = bookResults.Item2;
                var segmentResults = bhlProvider.SegmentSelectForAuthorIDPaged((int)creatorId, model.PartPage, model.NumPerPage, model.Sort);
