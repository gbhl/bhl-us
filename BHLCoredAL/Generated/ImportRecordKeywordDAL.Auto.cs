...
namespace MOBOT.BHL.DAL
{
	partial class ImportRecordKeywordDAL : IImportRecordKeywordDAL
	{
 		#region ===== SELECT =====
			...
			int importRecordKeywordID )
		{
				...
				CustomSqlHelper.CreateInputParameter("ImportRecordKeywordID", SqlDbType.Int, null, false, importRecordKeywordID)))
			{
				...
				{
					CustomGenericList<ImportRecordKeyword> list = helper.ExecuteReader(command);
					if (list.Count > 0)
					{
		...
		public CustomGenericList<CustomDataRow> ImportRecordKeywordSelectAutoRaw(
			SqlConnection sqlConnection, 
			SqlTransaction sqlTransaction, 
			int importRecordKeywordID)
		{
