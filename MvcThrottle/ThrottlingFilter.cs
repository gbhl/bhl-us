...
namespace MvcThrottle
{
    public class ThrottlingFilter : ActionFilterAttribute, IActionFilter
    {
        ...
        protected virtual string ComputeThrottleKey(RequestIdentity requestIdentity, RateLimitPeriod period)
        {
            ...
            /// Added Sept 2025 to ignore IP address when throttling specific User Agents
            bool throttledAgent = false;
            if (Policy.UserAgentRules != null && !string.IsNullOrEmpty(requestIdentity.UserAgent))
            {
                var rules = Policy.UserAgentRules.Where(x => requestIdentity.UserAgent.IndexOf(x.Key, 0, StringComparison.InvariantCultureIgnoreCase) != -1).ToList();
                if (rules.Any() && Policy.UserAgentThrottling) throttledAgent = true;
            }

            //if (Policy.IpThrottling)
            if (Policy.IpThrottling && !throttledAgent) // Ignore IP address throttling for throttled User Agents
