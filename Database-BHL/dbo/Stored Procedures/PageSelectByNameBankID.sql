...
SELECT	ni.IdentifierValue AS NameBankID, nr.ResolvedNameString,
		...
		REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(its.ImageServerUrlFormat, '{0}', ''), '{1}', ''), '{2}', ''), '{3}', 'http://www.biodiversitylibrary.org/pageimage/' + CONVERT(nvarchar(20), p.PageID)), '{4}', 'http://www.biodiversitylibrary.org/pageimage/' + CONVERT(nvarchar(20), p.PageID)), '&amp;', '&') AS ImageURL,
