...
SELECT @TotalAuthors = COUNT(*) FROM (
	SELECT	v.AuthorID
	FROM	dbo.AuthorName n WITH (NOLOCK)
			INNER JOIN dbo.TitleAuthorView v WITH (NOLOCK) ON n.AuthorID = v.AuthorID
	WHERE	v.IsActive = 1
...
INSERT		#Author
SELECT		v.AuthorID,
			ISNULL(n.FullName, '') + ISNULL(Numeration, '') + ISNULL(Unit, '') + ISNULL(v.Title, '') +
